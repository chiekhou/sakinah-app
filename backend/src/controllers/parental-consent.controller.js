...
class ParentalConsentController {
  ...
  async getConsentInfo(req, res) {
    try {
      ...
      // ── Solution alternative (page HTML web, si le parent n'a pas l'app) ──────────────
      // const renderError = (message) => {
      //   res.status(400).send(`<!DOCTYPE html>...<p class="msg">${message}</p>...`);
      // };
      ...
      // if (new Date() > new Date(consent.consent_token_expires)) return renderError("Ce lien a expiré.");
      ...
      // res.send(`<!DOCTYPE html>...<form method="POST" action="${apiUrl}/api/auth/confirm-parental-consent/${token}">...`);
      // ────────────────────────────────────────────────────────────────────────────────────

      // Solution Flutter : retourner les infos en JSON → deep link sakinah:// ouvre ParentConfirmationScreen
      if (!consent) {
  ...
  async confirmConsent(req, res) {
    try {
      ...
      // <html lang="fr">...<h1>Consentement confirmé !</h1>
      // <p>Le compte de ${child.username} est maintenant actif.
      // Téléchargez Sakinah pour gérer le compte de votre enfant.</p>...`);
      // ────────────────────────────────────────────────────────────────────────────────────

      // Solution Flutter : retourner JSON + JWT → ParentConfirmationScreen navigue vers le dashboard
